...
// import flipkartCategories from "./category.js";

const flipkartCategoriesdone = [
  {
    category: "Fashion",
    subcategory: "mens",
    url: "https://www.flipkart.com/search?q=mens+clothing",
    selectors: {
      item: "div[data-id]",
      name: "a.atJtCj",
      quantity: "div.yZLfgx",
  ...
  url:"https://www.flipkart.com/search?q=furniture",
  selectors:{
    item:"div.RGLWAk",
    name:"a.pIpigb",
...
},
{
  ...
  url:"https://www.flipkart.com/search?q=skincare",
  selectors:{
    item:"div.RGLWAk",
    name:"a.pIpigb",
    quantity:"div.MKiFS6",
    ...
    const newProducts = await page.evaluate((config) => {
      ...
      return Array.from(document.querySelectorAll(config.selectors.item))
        .map((item) => {
          ...
          let details = detailsEl?.innerText || null;

          if (details) {
            details = details.replace(/Size\s*/i, "").trim();
          }
